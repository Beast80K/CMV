...
export async function GET(NextRequest) {
	...
	try {
		let { data, status, statusText, headers } = await AxiosInstance.get('/coin_markets',
			{
				params: {
					"vs_currency": NextRequest.nextUrl.searchParams.get('currency'),
					"locale": NextRequest.nextUrl.searchParams.get('locale'),
					"ids": NextRequest.nextUrl.searchParams.get('ids') || ""
				}
			}
		)
